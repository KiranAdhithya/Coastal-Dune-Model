...
CInitSurfAlea::CInitSurfAlea(const dunepar& par, string prefix)
{
    ...
    m_nodes[1] = par.getdefault(prefix+"alea.nodes.m", 1);
}
...
//        if (iNY < 16) {
        ...
        for (int i = 0; i < m_nodes[0]; i++) {
            int x = rand() % (iNX - (m_xmin + 10)) + m_xmin; //m_xmin+30;
            int y = rand() % iNY;
            array(x,y) = 0.1 * m_rho_max;
        }
    }
}
...
void CInitSurfAlea::init_2d_vec(TFktVec& array)
{
    ...
    if (iNY < 8) {
        // 2D
        int x = m_xmin+25;//0.5 * iNX;
        int y = 0.5 * iNY;
        for (int k = 0; k < 2; k++) {
            //for( int y= 0; y< iNY; ++y ){
            array(x,y)[k] = (m_nodes[k] > 0 ? 0.2 : 0) * m_rho_max;
            //}
        }
    } else {
        // 3D : random seeding
